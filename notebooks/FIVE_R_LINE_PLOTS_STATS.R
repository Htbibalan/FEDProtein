...
#########################################################################################################################################################################
########################################################## FEAST NUMBER REDO WITH ORDER1 DAYS POSTHOC##############################################################
#######################################################################################################################################################################
...
write.csv(descriptive,
          file.path(outdir, "feastnumber_descriptives_by_Order_Sex_phase_day.csv"),
          row.names = FALSE)
...
#########################################################################################################################################################################
########################################################## FEAST FREQUENCY REDO WITH ORDER1 DAYS POSTHOC##############################################################
#######################################################################################################################################################################
...
descriptive <- long %>%
  ...
  summarise(n = sum(!is.na(value)),
            ...
            .groups = "drop")
write.csv(descriptive,
          file.path(outdir, "feast_freq_descriptives_by_Order_Sex_phase_day.csv"),
          row.names = FALSE)
...
anova_tab <- afex::nice(fit, es = "pes", correction = "GG")
write.csv(anova_tab, file.path(outdir, "feast_freq_ANOVA_GG_with_Sex.csv"), row.names = FALSE)
...
# Save the grid (helps inspect labels; day may appear as X0..X6)
grid <- as.data.frame(em)[, c("phase","day","Order","Sex")]
write.csv(grid, file.path(outdir, "EMM_grid_feast_freq_with_Sex.csv"), row.names = FALSE)
...
#########################################################################################################################################################################
########################################################## FEAST SIZE REDO WITH ORDER1 DAYS POSTHOC##############################################################
#######################################################################################################################################################################
...
descriptive <- long %>%
  ...
  summarise(n = sum(!is.na(value)),
            ...
            .groups = "drop")
write.csv(descriptive,
          file.path(outdir, "feast_size_descriptives_by_Order_Sex_phase_day.csv"),
          row.names = FALSE)
...
anova_tab <- afex::nice(fit, es = "pes", correction = "GG")
write.csv(anova_tab, file.path(outdir, "feast_size_ANOVA_GG_with_Sex.csv"), row.names = FALSE)
...
# Save the grid (helps inspect labels; day may appear as X0..X6)
grid <- as.data.frame(em)[, c("phase","day","Order","Sex")]
write.csv(grid, file.path(outdir, "EMM_grid_feast_size_with_Sex.csv"), row.names = FALSE)
